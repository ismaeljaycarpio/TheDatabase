...
function insertHTML(sHTML)
  ...
  if(oSel.parentElement) {
    if(isIE9) {
      ...
      //for some reason pasteHTML always add double &nbsp; into range in IE9
  ...
  if(evt.keyCode==13)
    ...
    if(edt.returnKeyMode==1)
      ...
      if(oSel.parentElement)
        {
        edt.saveForUndo();//Save for Undo
        if(GetElement(oSel.parentElement(),"LI")) 
          {
            //do nothing, just browser's behavior.
          }
        else if(GetElement(oSel.parentElement(),"FORM"))
          {
          var oSel=document.selection.createRange();
          oSel.pasteHTML('<br>');
...
function tbAction(tb, id, edt, sfx) {
  ...
  switch(btn) {
    ...
    case "btnCharacters": e.hide();if(!e.customDialogShow("Characters")) modelessDialogShow(e.scriptPath+"characters.htm",495,162); break;
...
function ddAction(tb, id, edt, sfx) {
  ...
  switch(btn) {
    case "btnPreview1":
      setActiveEditor(oN);
      if(!e.customDialogShow("Preview1")) modalDialogShow(e.scriptPath+"preview.htm?w=640&h=480",640,480); 
      break;
    case "btnPreview2": 
      setActiveEditor(oN);
      if(!e.customDialogShow("Preview2")) modalDialogShow(e.scriptPath+"preview.htm?w=800&h=600",800,600); 
      break;
    case "btnPreview3": 
      setActiveEditor(oN);
      if(!e.customDialogShow("Preview3")) modalDialogShow(e.scriptPath+"preview.htm?w=1024&h=768",1024,768); 
      break;
    ...
    case "btnListFormatting": if(!e.customDialogShow("ListFormatting")) modelessDialogShow(e.scriptPath+"list.htm",320,335); break;
