...
function InnovaEditor(oName)
  ...
  /*** /icons related ***/
    ...
    this.useBR=false;
    this.useDIV=true;
    this.returnKeyMode = -1; //0:default, 1:div, 2:br, 3:p    
...
function openStyleSelect()
  ...
  var sHTML="";
  sHTML+="<div unselectable=on style='margin:5px;margin-top:0px;margin-right:1px' align='left'>";
    ...
    if(evt.keyCode==13)
        {
            ...
            if(evt.shiftKey) {
              ...
              if(obj.returnKeyMode==3) {
                  
                  //insert p
                  $applyReturnEvent(evt, obj, "p");
                  
              } else if(obj.returnKeyMode==2) {
...
function $applyReturnEvent(evt, obj, tag) {
    ...
    if(selElm) {
        ...
        if(li) {
          //move cursor outside li and insert
          var range = sel.getRangeAt(0);
          range.setStartAfter(li);
          range.setEndAfter(li);
          //insert li instead
          var docFrag = range.createContextualFragment("<li><br /></li>");
    ...
    if(el && (
       ...
       el.tagName=="P" || el.tagName=="DIV" || el.tagName=="PRE")) {                   
    ...
    if(el) { 
      ...
      var cnt = docFrag.firstChild; //save the reference to docFrag content because after insert, docFrag will be empty
...
function tbAction(tb, id, edt, sfx) {
  ...
  switch(btn) {
    ...
    case "btnBookmark": e.hide(); if(!e.customDialogShow("Bookmark")) modelessDialogShow(e.scriptPath+"bookmark.htm",245,216); break;
    case "btnHyperlink": e.hide(); if(!e.customDialogShow("Hyperlink")) modelessDialogShow(e.scriptPath+"hyperlink.htm",380,220); break;
    case "btnImage": e.hide(); if(!e.customDialogShow("Image")) modelessDialogShow(e.scriptPath+"image.htm",440,351); break;
    case "btnFlash": e.hide(); if(!e.customDialogShow("Flash")) modelessDialogShow(e.scriptPath+"flash.htm",340,275); break;
    case "btnYoutubeVideo": e.hide(); if(!e.customDialogShow("YoutubeVideo")) modelessDialogShow(e.scriptPath+"youtube_video.htm",340,270); break;
    case "btnMedia": e.hide(); if(!e.customDialogShow("Media")) modelessDialogShow(e.scriptPath+"media.htm",340,272); break;
    ...
    case "btnAutoTable": e.hide(); if(!e.customDialogShow("AutoTable")) modelessDialogShow(e.scriptPath+"table_format.htm",320,225); break;
    case "btnAbsolute": e.makeAbsolute(); break;
    case "btnCharacters": e.hide(); if(!e.customDialogShow("Characters")) modelessDialogShow(e.scriptPath+"characters.htm",495,160); break;
...
function ddAction(tb, id, edt, sfx) {
  ...
  switch(btn) {
    case "btnPreview1":
      setActiveEditor(edt);
      if(!e.customDialogShow("Preview1")) modalDialogShow(e.scriptPath+"preview.htm",640,480); 
      break;
    case "btnPreview2": 
      setActiveEditor(edt);
      if(!e.customDialogShow("Preview2")) modalDialogShow(e.scriptPath+"preview.htm",800,600); 
      break;
    case "btnPreview3": 
      setActiveEditor(edt);
      if(!e.customDialogShow("Preview3")) modalDialogShow(e.scriptPath+"preview.htm",1024,768); 
      break;
    case "btnTextFormatting": if(!e.customDialogShow("TextFormatting")) modelessDialogShow(e.scriptPath+"text1.htm",511,490); break;
    case "btnParagraphFormatting": if(!e.customDialogShow("ParagraphFormatting")) modelessDialogShow(e.scriptPath+"paragraph.htm",460,284); break;
    case "btnListFormatting": if(!e.customDialogShow("ListFormatting")) modelessDialogShow(e.scriptPath+"list.htm",305,335); break;
    case "btnBoxFormatting": if(!e.customDialogShow("BoxFormatting")) modelessDialogShow(e.scriptPath+"box.htm",498,395); break;
    case "btnCssText": if(!e.customDialogShow("CssText")) modelessDialogShow(e.scriptPath+"styles_cssText.htm",360,332); break;
    case "btnCssBuilder": if(!e.customDialogShow("CssBuilder")) modelessDialogShow(e.scriptPath+"styles_cssText2.htm",430,445); break;
...
function fixWord()
    ...
    //check if word content.
    var isWord = _isWordContent(idSource.innerHTML); 
    if(!isWord) {
      var cnt = idSource.innerHTML;
