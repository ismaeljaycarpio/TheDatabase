...
<script runat="server">
    ...
    Protected Sub SecurityValidation()
        ' check whether user is authorized to view this page
        Dim bAuthorized As Boolean = False
        
        Dim sUsername As String = ""
        If Not Session("LoginInfo") Is Nothing Then
            sUsername = Session("LoginInfo")("UserName")
        End If
        
        'If sUsername.Length > 0 Then
        '    ' check whether current user specified in the web.config
        '    Dim sAllowedUsers As String()
        '    sAllowedUsers = System.Configuration.ConfigurationManager.AppSettings("AssetManagerUser").Split(",")
            
        '    For Each sAllowedUser As String In sAllowedUsers
        '        If sAllowedUser.ToUpper() = sUsername.ToUpper() Then
        '            bAuthorized = True
        '        End If
        '    Next
        ...
        If sUsername.Length > 0 Then
            If (DBGurus.UserHasRole(sUsername, 1)) Then
                bAuthorized = True
            End If
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ' Unauthorized user should not use this page
        SecurityValidation()
    ...
    Function ShowCheckBox(ByVal sUrl As String) As String
        ...
        Else
            'This is a File
            sHTML = String.Concat("<input name=""chkSelect"" type=""checkbox"" title=""", GetGlobalResourceObject("TIAR3", "DS:Text"), """ />")
...
<head id="Head1" runat="server">
    <title id="idTitle" meta:resourcekey="idTitle" runat="server"></title><meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7"/>
    <style type="text/css">
    ...
    function selectImage(nIndex)
        {
            if (document.getElementById("hidFileUrl"+nIndex) != null)
            {
                var s=document.getElementById("hidFileUrl"+nIndex).value;
                document.getElementById("txtURL").value=s;
                
                if (document.getElementById("hidThumbnail") != null)
                {
    ...
    function doOK()
        ...
        else window.opener.setAssetValue(sURL);
        self.close();
        }
   </script>
</head>
